...
async fn handle_connection(mut broker: Sender<Event>, stream: TlsStream<TcpStream>) {
    ...
    loop {
        match protocol_reader::read_message(&mut broker, &peer_id, &mut reader).await {
            Ok(true) => {
...
mod protocol_reader {
    use crate::constants::{
        ConnectionError, ConnectionErrorKind, PacketNonce, MessageLength, ClientAction, PeerId, ConferenceId, ConferenceEncryptionSalt, ConferenceJoinSalt, PasswordHash
    };
    use crate::broker::{Event, Sender};
    use super::deobfuscate_conference_id;
